import pandas as pd, sys, ctypes, pathlib as pl, tkinter as tk
...
def treeview_adjust_table(display_data:pd.DataFrame, tree_view:Treeview, scroll_bar:ttk.Scrollbar, strava_data:StravaData):
    ...
    for i in list(display_data.columns):
        tree_view.heading(i, text=i, command=lambda col=i: (strava_data.sort_values(col), insert_table(tree_view, scroll_bar, strava_data)))
...
def insert_table(tree_view, scroll_bar, strava_data:StravaData):
    """Insert table with filtered or unfiltered results packed, and initialize the treeview."""
...
def retrieve_entry(column_name:tk.Entry, operator:tk.Entry, value_entry:tk.Entry, strava_data:StravaData, reset=False):
    ...
    strava_data.filter_results(column_name, operator, value, reset) # noqa
...
def init_buttons_config(tree_scroll:list, input_boxes:list, window, cycling_container:list):
    ...
    button_config=[
        ...
        },
        {
            ...
            "type": "single",
            "text": "Insert Table",
            "command": lambda:insert_table(tree_view, scroll_bar, cycling_container[0])
        },
        {
            "parent": window,
            "type": "group",
